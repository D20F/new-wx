<template>
    <view>
        <view class="order-time" @tap="showCalendar()" v-if="!modal">
            <view class="time-viewer" v-if="singleDate"
                >{{ choiceDate[0].year }}年{{ choiceDate[0].month }}月{{
                    choiceDate[0].day
                }}日</view
            >
            <view class="time-viewer" v-else>
                ...
                <text class="date-wrappper"
                    >{{ choiceDate[0].month }}月{{ choiceDate[0].day }}日</text
                >
                <text
                    class="goInHotel2"
                    v-if="
                ...
                <text class="date-wrappper"
                    >{{ choiceDate[1].month }}月{{ choiceDate[1].day }}日</text
                >
                <text
                    class="goInHotel2"
                    v-if="
                        choiceDate[choiceDate.length - 1].year +
                            '' +
                            choiceDate[choiceDate.length - 1].month +
                            '' +
                            choiceDate[choiceDate.length - 1].day ==
                        tomorrow
                    "
                >
                    明天
...
export default {
    ...
    methods: {
        ...
        getDayType(data) {
            return data.re != this.today &&
                data.re != this.tomorrow &&
                data.re != this.afterTomorrow
                ? data.act.subject
                : "";
        },
        ...
        getRectList: async function () {
            if (!this.layerTop || this.layerTop < 0)
                this.layerTop = await this.getLayerTop();
            ...
            if (this.monthTitleRectList && this.monthTitleRectList.length > 0) {
                if (this.monthTitleRectList[0].top == 0) {
                    this.monthTitleRectList =
                        await this.getMonthTitleRectList();
                    isInitRectData = false;
                }
            } else {
                isInitRectData = false;
            }
            if (isInitRectData == false)
        ...
        showCalendar: function () {
            ...
            this.bak_choiceDateArr = JSON.parse(
                JSON.stringify(this.choiceDateArr)
            );
        ...
        hideCalendar: function (isBtnClick) {
            ...
            this.choiceDateArr = JSON.parse(
                JSON.stringify(this.bak_choiceDateArr)
            );
            // console.log('h 4');
        ...
        dateData: function () {
            ...
            this.today =
                "" + year + "-" + this.pad(month, 2) + "-" + this.pad(day, 2);
            this.tomorrow =
                "" +
                year +
                "-" +
            ...
            if (endDate) {
                ...
                if (endDate.getTime() < getDateTime)
                ...
                for (let j = 0; j < mList.length; j++) {
                    ...
                    for (let k = 0; k < t_days[j]; k++) {
                        ...
                        if (dayscNow < daysCount) {
                            ...
                            var subject =
                                this.isHoliday(yearList[i], monthAndDay) ||
                                this.isWorkday(yearList[i], monthAndDay) ||
                                "";

                            var none = false;
                            let newDateFormate = new Date(
            ...
            let startIndex1 =
                start_year == year
                    ? start_month - month
                    : start_month + (12 - month);
            let startIndex2 = start_day - 1;

            //当前选中的结束时间坐标
            let endIndex1 =
                end_year == year ? end_month - month : end_month + (12 - month);
            ...
            if (startIndex1 == endIndex1 && endIndex2 - startIndex2 == 1) {
                if (dataAll2[startIndex1][startIndex2 + 1]) {
                    ...
                    this.choiceDate.push(
                        dataAll2[startIndex1][startIndex2 + 1]
                    );
                } else {
                    islastDay = true;
                }
        ...
        selectday: function (index, indexs, isUserClick) {
            ...
            if (
                this.dateFlag.date &&
                curDate.dateTime < this.dateFlag.date.dateTime
            ) {
                // console.log("004");
                ...
                // console.log("005")
                if (
                    this.dateFlag.index == index &&
                    this.dateFlag.indexs == indexs
                ) {
                    return;
                }
                curDate.act.tip = "离店";
                ...
                this.date.forEach(function (dataItems) {
                    dataItems.forEach(function (dataItem) {
                        if (
                            dataItem.dateTime > dateFlagDateTime &&
                            dataItem.dateTime < choiceDateTime
                        ) {
                ...
                this.choiceDate[1] =
                    that.choiceDateArr[that.choiceDateArr.length - 1];
                //
                if (nonFlag) {
                    var that = this;
                    this.date.forEach(function (dataItems) {
                ...
                this.date.forEach(function (dataItems) {
                    dataItems.forEach(function (dataItem) {
                        ...
                        if (
                            dataItem.dateTime !=
                            that.date[index][indexs].dateTime
                        ) {
                            dataItem.selected = 0;
                            dataItem.act.tip = "";
                        } else {
        ...
        submitbtn: function () {
            ...
            this.choiceDate[1] =
                this.choiceDateArr[this.choiceDateArr.length - 1];
            this.dayCount2 = "共" + this.dayCount + "晚";
             ...
             */
            this.$emit("change", {
                choiceDate: this.singleDate
                    ? this.choiceDate[0]
                    : this.choiceDate, //如果是单个日期方式，则只导出数组第一个
                dayCount: this.dayCount,
            });
        },
    },
