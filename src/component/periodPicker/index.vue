		...
		<view class="u-datetime-picker">
			...
			<view class="u-picker-body">
				<picker-view v-if="mode == 'time'" :value="valueArr" @change="change" class="u-picker-view" @pickstart="pickstart" @pickend="pickend">
					...
					<picker-view-column v-if="!reset && params.hourEnd">
						<view class="u-column-item" v-for="(item, index) in hourEnds" :key="index">
							{{ formatNumber(item) }}
							<text class="u-text" v-if="showTimeTag">时</text>
						</view>
					</picker-view-column>
...
export default {
	...
	methods: {
		...
		setHourEnds() {
			this.hourEnds = this.generateArray(0, 23);
			this.valueArr.splice(this.valueArr.length - 1, 1, this.getIndex(this.hourEnds, this.hourEnd));
		},
		setMinutes() {
		...
		change(e) {
			...
			if (this.mode == 'time') {
                ...
                i++; //多了一个至 多自增一次
				if (this.params.hourEnd) this.hourEnd = this.hourEnds[this.valueArr[i++]];
		...
		getResult(event = null) {
			...
			if (this.mode == 'time') {
				...
				if (this.params.hourEnd) result.hourEnd = this.formatNumber(this.hourEnd || 0);
